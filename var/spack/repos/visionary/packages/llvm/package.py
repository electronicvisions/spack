...
class Llvm(CMakePackage, CudaPackage, LlvmDetection, CompilerPackage):
    ...
    # version constraints changed compared to EBRAINS version
    ...
    # 0001-Tooling-Fully-qualify-template-parameters-of-nested-.patch from above
    patch('llvm16-0002-libclang-Add-support-for-obtaining-fully-qualified-n.patch', when='@16 +visionary', level=1)
    patch('llvm16-0003-libclang-Add-option-to-keep-whitespace-when-tokenizi.patch', when='@16 +visionary', level=1)
    patch('llvm16-0004-libclang-WIP-Allow-visiting-of-implicit-declarations.patch', when='@16 +visionary', level=1)
    # 0005-libclang-WIP-Fix-get_tokens-in-macro-expansion.patch from above
    # llvm14-public-ClangToolingCommonOptionsParser.patch from above

    # 0001-Tooling-Fully-qualify-template-parameters-of-nested-.patch from above
    patch('llvm17-0002-libclang-Add-support-for-obtaining-fully-qualified-n.patch', when='@17: +visionary', level=1)
    patch('llvm17-0003-libclang-Add-option-to-keep-whitespace-when-tokenizi.patch', when='@17: +visionary', level=1)
    patch('llvm17-0004-libclang-WIP-Allow-visiting-of-implicit-declarations.patch', when='@17: +visionary', level=1)
    # 0005-libclang-WIP-Fix-get_tokens-in-macro-expansion.patch from above
    # llvm14-public-ClangToolingCommonOptionsParser.patch from above
    ...
    # begin EBRAINS
    # add missing concept declaration CursorKind to clang's Python bindings
    patch(
        "https://github.com/llvm/llvm-project/commit/8b322895207c34b434698954dc67404e0bbf8d8e.patch?full_index=1",
        sha256="173050a7f08c01bbc18d1145f455cac827754d0436befc2afd1add86e884fac0",
        when="@15:16",
    )
    # Add some Kinds existing in Index.h but missing in cindex.py (dropped release note changes)
    patch(
        # based on https://github.com/llvm/llvm-project/commit/eb264d825beb048c6e673ddaf5aca069511fcfb3.patch?full_index=1
        "llvm15-clang-python-missing-kinds.patch",
        when="@15:18",
    )
    # end EBRAINS
