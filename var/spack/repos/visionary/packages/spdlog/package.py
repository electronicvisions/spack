...
class Spdlog(CMakePackage):
    ...
    depends_on("fmt@5.3:9", when="+fmt_external")
    depends_on("fmt@7:9", when="@1.7: +fmt_external")
    depends_on("fmt@8:9", when="@1.9: +fmt_external")
    depends_on("fmt@9:9", when="@1.11: +fmt_external")
    # end VISIONS
