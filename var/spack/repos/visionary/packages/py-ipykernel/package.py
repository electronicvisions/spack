...
class PyIpykernel(PythonPackage):
    ...
    def install_data(self):
        ...
        # Create kernel with absolute path to used python version and not the symlink in the virtual
        # environment. This ensures that the correct site-packages are available during run-time.
        python_exe = os.path.realpath(self.spec['python'].command.path)

        # Call ipykernel installation using the resolved binary
        real_python = Executable(python_exe)
        real_python("-m", "ipykernel", "install", "--prefix=" + self.prefix)
